...
class OpExValidator:
    ...
    def print_validation_report(self, report: ValidationReport) -> None:
        ...
        print("\n" + "=" * 70)
        print("OPEX PINNING STRATEGY VALIDATION REPORT")
        print("=" * 70)

        print("\n[1] PINNING FREQUENCY ANALYSIS")
        print("-" * 40)
        print(f"  OpEx Events: {report.pinning.total_opex_events}")
        print(f"  Pinning Detected: {report.pinning.pinning_detected} ({report.pinning.pinning_rate:.1%})")
        print(f"  Avg Distance (NORMAL): {report.pinning.avg_distance_normal:.2%}")
        print(f"  Avg Distance (PINNING): {report.pinning.avg_distance_pinning:.2%}")
        print(f"  Statistical Test p-value: {report.pinning.pinning_vs_normal_pvalue:.4f}")
        print(f"  Hypothesis Confirmed: {'YES' if report.pinning_hypothesis_confirmed else 'NO'}")

        print("\n[2] PHASE RETURNS ANALYSIS")
        print("-" * 40)
        print(f"  NORMAL avg return: {report.phase_returns.normal_avg_return:.4%} (n={report.phase_returns.normal_count})")
        print(f"  PINNING avg return: {report.phase_returns.pinning_avg_return:.4%} (n={report.phase_returns.pinning_count})")
        print(f"  POST_OPEX avg return: {report.phase_returns.post_opex_avg_return:.4%} (n={report.phase_returns.post_opex_count})")
        print(f"  Phase Effect Confirmed: {'YES' if report.phase_effect_confirmed else 'NO'}")

        print("\n[3] GEX PREDICTIVENESS")
        print("-" * 40)
        print(f"  GEX-Price Correlation: {report.gex_predictive.gex_price_correlation:.3f}")
        print(f"  Correlation p-value: {report.gex_predictive.gex_correlation_pvalue:.4f}")
        print(f"  High GEX avg return: {report.gex_predictive.high_gex_avg_return:.4%}")
        print(f"  Low GEX avg return: {report.gex_predictive.low_gex_avg_return:.4%}")
        print(f"  GEX Predictive: {'YES' if report.gex_predictive_confirmed else 'NO'}")

        print("\n[4] SUCCESS CRITERIA")
        print("-" * 40)
        print(f"  Sharpe >= {self.MIN_SHARPE}: {'PASS' if report.meets_sharpe_target else 'FAIL'}")
        print(f"  Win Rate >= {self.MIN_WIN_RATE:.0%}: {'PASS' if report.meets_win_rate_target else 'FAIL'}")
        print(f"  Max DD <= {self.MAX_DRAWDOWN:.0%}: {'PASS' if report.meets_drawdown_target else 'FAIL'}")

        print("\n" + "=" * 70)
        print(f"STRATEGY VIABLE: {'YES' if report.strategy_viable else 'NO'}")
        print("=" * 70)
