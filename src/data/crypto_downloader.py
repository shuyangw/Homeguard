"""
Alpaca Crypto Data Downloader Module.

Provides a reusable CryptoDownloader class for downloading historical OHLCV data
from Alpaca Crypto API with robust error handling, multithreading, and schema compliance.

Usage:
    from src.data.crypto_downloader import CryptoDownloader, Timeframe

    downloader = CryptoDownloader(start_date='2020-01-01')
    result = downloader.download_symbols(['BTC/USD', 'ETH/USD'], timeframe=Timeframe.MINUTE)
"""
...
# Thread-local storage for Alpaca crypto clients
...
_OUTPUT_DIR_SUFFIX = {
    Timeframe.MINUTE: "crypto_1min",
    Timeframe.HOUR: "crypto_1hour",
    Timeframe.DAY: "crypto_1day",
}


def normalize_symbol(symbol: str) -> str:
    """
    Convert crypto symbol from API format to filesystem-safe format.

    Args:
        symbol: Symbol in API format (e.g., 'BTC/USD')

    Returns:
        Filesystem-safe symbol (e.g., 'BTC_USD')
    """
...
def denormalize_symbol(symbol: str) -> str:
    """
    Convert crypto symbol from filesystem format back to API format.

    Args:
        symbol: Symbol in filesystem format (e.g., 'BTC_USD')

    Returns:
        API format symbol (e.g., 'BTC/USD')
    """
    return symbol.replace('_', '/')
...
def _get_crypto_client() -> CryptoHistoricalDataClient:
    """Get thread-local Alpaca crypto client."""
    if not hasattr(_thread_local, 'crypto_client'):
        _thread_local.crypto_client = CryptoHistoricalDataClient(API_KEY, API_SECRET)
    return _thread_local.crypto_client
...
class CryptoDownloader:
    """
    Download historical OHLCV data from Alpaca Crypto API.
    ...
    DEFAULT_START_DATE = '2020-01-01'  # Crypto data available from 2020
    ...
    def _download_single_symbol(
        ...
        Args:
            symbol: Crypto symbol to download (e.g., 'BTC/USD')
        ...
        fs_symbol = normalize_symbol(symbol)
        ...
        for attempt in range(1, self.MAX_RETRIES_PER_SYMBOL + 1):
            try:
                ...
                # Crypto API returns: timestamp, open, high, low, close, volume, trade_count, vwap
                required_columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume', 'trade_count', 'vwap']

                # Check for missing columns and fill with NaN if needed
                for col in required_columns:
                    if col not in df.columns:
                        logger.warning(f"[{tid}] {symbol}: Missing column '{col}', filling with NaN")
                        df[col] = float('nan')
                ...
                for (year, month), group in df.groupby(['year', 'month']):
                    partition_dir = output_dir / f'symbol={fs_symbol}' / f'year={year}' / f'month={month}'
    ...
    def _download_batch(
        ...
        Args:
            symbols: List of symbols to download (e.g., ['BTC/USD', 'ETH/USD'])
            timeframe: Timeframe for the data
    ...
    def download_symbols(
        ...
        """
        Download historical data for a list of crypto symbols.

        Args:
            symbols: List of crypto symbols to download (e.g., ['BTC/USD', 'ETH/USD'])
        ...
        logger.info("=" * 60)
        logger.info(f"ALPACA CRYPTO DATA DOWNLOADER - {timeframe.value.upper()} DATA")
        logger.info("=" * 60)
