...
| **Fills-level trade log persisted** | **12 / strategy-pipeline.md** | **Fills (position changes, not just daily P&L) MUST exist on disk: EITHER the single-window `trades.csv` OR a run-scoped `runs/<run_id>/manifest.csv` with a non-empty `trades_oos.csv.gz`. A walk-forward VERDICT MUST have the `trades_oos.csv.gz` OOS concatenation (the actual gated fills), not just a representative single-pass log.** | **Do NOT mark the phase complete. Re-run with trade logging ON and verify the artifact exists (`trades.csv` for a single-window verdict, or the manifest + non-empty `trades_oos.csv.gz` for a walk-forward verdict).** |

**Trade logging is MANDATORY and you must VERIFY THE ARTIFACT, not just instruct it (this was missed on 2026-07-19: custom walk-forward runners aggregated to daily returns + logged return-streams but never persisted a fills-level `trades.csv`, and no primary logged backtest was run).** Every simulated run persists its fills -- per-window, per-config, and per inner optimizer probe -- so a walk-forward VERDICT MUST carry the `trades_oos.csv.gz` OOS concatenation (the actual gated fills) under its run-scoped sink `output/backtests/<strategy>/runs/<run_id>/`, alongside a `manifest.csv`; a single-window verdict instead persists a plain `output/backtests/<strategy>/<start>_to_<end>/trades.csv`. For walk-forward runs, the per-window files (`wNN_<leg>_trades.csv.gz`) are the FULL-window runs (train + test), and `trades_oos.csv.gz` is those windows sliced to their out-of-sample `[test_start, test_end)` segments and concatenated -- so `trades_oos` is the fills matching the gated OOS return series. A representative single-pass log does NOT satisfy a walk-forward verdict. If a strategy is gated via a CUSTOM walk-forward runner (S&P-relative, R-multiple, or spread-book aggregation to daily returns) that emits neither the manifest + `trades_oos.csv.gz` nor a `trades.csv`, you MUST re-run it through the trade-logging path (config-driven `run_fx_backtest`/`backtest_runner` with `log_trades=True`/`save_trades: true`, or `run_futures_backtest(..., log_trades=True)`) and confirm the fills artifact exists BEFORE marking the phase complete. "Return-stream logged" (daily P&L in `output/experiments.duckdb`) does NOT satisfy this -- the fills-level log is required for the Section 12 MAE/MFE, hold-time, capacity, and regime-attribution diagnostics.
