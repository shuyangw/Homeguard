...
| **Fills-level trade log persisted** | **12 / strategy-pipeline.md** | **A `trades.csv` (fills/position changes, not just daily P&L) MUST exist on disk for the strategy's primary/representative full-sample backtest** | **Do NOT mark the phase complete. Re-run a primary full-sample backtest with trade logging ON and verify the artifact exists.** |

**Trade logging is MANDATORY and you must VERIFY THE ARTIFACT, not just instruct it (this was missed on 2026-07-19: custom walk-forward runners aggregated to daily returns + logged return-streams but never persisted a fills-level `trades.csv`, and no primary logged backtest was run).** Per-window walk-forward runs MAY suppress trade logging, but the PRIMARY/representative backtest for every gated strategy MUST persist a fills-level trade log. If a strategy is gated via a CUSTOM walk-forward runner (S&P-relative, R-multiple, or spread-book aggregation to daily returns) that does not emit `trades.csv`, you MUST additionally run one primary full-sample backtest through the trade-logging path (config-driven `run_fx_backtest`/`backtest_runner` with `log_trades=True`/`save_trades: true`, or `run_futures_backtest(..., log_trades=True)`) and confirm the `trades.csv` exists BEFORE marking the phase complete. "Return-stream logged" (daily P&L in `output/experiments.duckdb`) does NOT satisfy this -- the fills-level log is required for the Section 12 MAE/MFE, hold-time, capacity, and regime-attribution diagnostics.
