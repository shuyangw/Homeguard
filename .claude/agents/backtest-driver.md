...
- **Run-status logging** (ALWAYS for long/background runs): wrap the run in `RunStatus` (`src/utils/run_status.py`) so a killed run leaves a stale `RUNNING` sentinel + last heartbeat under `output/run_status/`. A `SIGKILL`'d process cannot self-log its death, so this is the ONLY way to later know a run died vs completed. Do not switch git branches or mutate the working tree while a long run is in flight. If a run turns up killed, read its `output/run_status/` file for the last-alive time before hypothesizing a cause.
