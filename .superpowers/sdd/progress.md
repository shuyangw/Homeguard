...
- Task 3: COMPLETE (commit 1eb4450, review Approved, 0 Critical/Important; 5 passed). Minor roll-up: (a) unmapped-currency branch (series_id None) untested -- only missing-file branch covered; (b) 3 zero-fallback branches could DRY to a helper.
- Task 4: COMPLETE (commit 24cb430, review Approved, 0 Crit/Imp; 5/5 incl 3 existing futures IDM unchanged). Minor roll-up: (a) compute_div_mult docstring still says unconditional KeyError; (b) fx_cluster_for cross precedence USD>EUR>JPY undocumented (EURJPY->eur_cross); (c) SGD grouped as EM (debatable; not in v1 universe).
- Task 5: COMPLETE (commit 1f0828f, review Approved, 0 Crit/Imp; 3/3). Minor roll-up: (a) dead `if denom<=0` guard unreachable (anti-overeng rule) -- REMOVE in final fix wave; (b) base_to_usd<=0 and vol_target<=0 zero-guard branches untested.
- Task 6: COMPLETE (commit 48a3e8b, review Approved, 0 Crit/Imp; 4/4 new + 29/29 costs regression, append-only verified). Minor roll-up: fx.py:67 continuation indent 1 col short (E128 cosmetic).
- Task 7: COMPLETE (impl 78d76c2 + fix 365e147, opus review Approved + fix re-review Approved; golden carry 2001.996 hand-verified; 4 tests incl NaN-gap regression). Fix: forward-hold NaN-input pairs on rebalance (was force-flatten -> NaN cost/equity corruption on real gap days). Roll-up: (a) redundant `not blown` guard (mirrors sibling); (b) NaN regression only covers px/q not forecast/vol; (c) FLAG for final review + Task 11 caveat: gap-SPANNING MTM move is dropped, not realized on reopen (prev_close holds NaN row) -- material for 2020 EURUSD multi-week outage; v1.1 candidate (forward-fill prev_close to realize accumulated move).
- Task 8: COMPLETE (commit 386fe5f, review Approved, 0 findings; 2/2 + 15/15 registry regression, additive verified). Note: FxValue test asserts columns only, not cap (per brief).
- Task 9: COMPLETE (commit 3cc6696, opus review Approved, 0 Crit/Imp; e2e + 24 FX + 32 routing pass; routing additive, _resolve_costs untouched, composition wiring + daily-vol unit convention verified). Minor roll-up: (a) _write_trade_log not try/wrapped (inconsistent w/ non-fatal registry); (b) session="ny"/cost_mult not surfaced in YAML. NOTE: implementer restored [macos] section to settings.ini LOCALLY (uncommitted, not in commit) so StandardReportGenerator runs here.
- Task 10: COMPLETE (commit 8423f18, review Approved, 0 Crit/Imp). Shared walkforward_common.py extracted VERBATIM (reviewer re-ran futures regression LIVE: 5 passed/1 pre-existing skip); FX walk_forward_fx runs both cost legs, stitches OOS, gates on stitched, asset_class=fx, trial_count=1; 2 FX tests pass. Minor roll-up: (a) _oos_returns_dated error msg reworded (inert; actually more-correct now shared); (b) --train-months no code-enforced 72 for FxValue (operator flag); (c) no unit test for FX _config_to_kwargs.
- Task 11 (real-data acceptance): DEFERRED to EC2/Windows -- cannot run on this Mac (no [macos] storage config; fx_1min/FRED data not local). Steps: build_fx_daily_cache -> run_fx_backtest fx_trend -> run_fx_walkforward (value w/ --train-months 72).
- ALL CODE TASKS 1-10 COMPLETE + REVIEWED. Next: final whole-branch review.
- FINAL whole-branch review (opus): 0 Critical, 4 Important (cross-cutting, missed by per-task reviews) + minors triaged. FIX WAVE (commit 8b82489, re-review opus READY-TO-MERGE-YES): #1 FxTrend present-pair (was KeyError on partial cache); #2 CALENDAR-day carry (was per-trading-day-bar /365 -> ~31% understatement); #3 per-pair cost tier (crosses were under-costed 2.5x major->minor); #4 dropped dead denom guard. Golden carry 2001.996 unchanged; +weekend-carry & missing-pair & cross-cost tests. Deferred v1.1: gap-spanning MTM realize-on-reopen (release-noted in spec). Consolidated FX surface: 35 passed / 1 skip (data-dep futures WF). 
- BRANCH CODE-COMPLETE + MERGE-READY (Tasks 1-10). Task 11 real-data acceptance PENDING on EC2/Windows. Next: finishing-a-development-branch (user decides merge/PR/push).
