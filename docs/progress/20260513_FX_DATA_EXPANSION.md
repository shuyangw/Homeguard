...
### New code
- **`C:\Users\qwqw1\Dropbox\cs\github\Homeguard\src\data\acquisition\plugins\massive_fx_flat.py`** (~250 LOC): standalone single-pass downloader. Not a `BaseDownloader` subclass because Massive's flat files are per-day-all-pairs, not per-symbol-per-day; symbol-iteration would re-download each 15 MB file once per ticker. Iterates days, fans out to per-symbol monthly buffers, atomic parquet write at month boundary. `ThreadPoolExecutor` parallelizes per-day S3 fetches within a month. Respects `skip_existing` for resumability.
- **`C:\Users\qwqw1\Dropbox\cs\github\Homeguard\scripts\data\download_fx.py`** (~100 LOC): thin CLI wrapper reading the universe CSV and dispatching to `download_pairs`. Args: `--tier`, `--symbol`, `--start`, `--end`, `--no-skip-existing`, `--dry-run`.
- **`C:\Users\qwqw1\Dropbox\cs\github\Homeguard\config\universes\fx-2026.csv`**: 7-row universe definition with `symbol, tier, added_date, effective_start_date, massive_ticker, notes` columns. Follows existing `C:\Users\qwqw1\Dropbox\cs\github\Homeguard\config\universes\sp500-2025.csv` / `russell1000-2025.csv` pattern.
- **`C:\Users\qwqw1\Dropbox\cs\github\Homeguard\tests\data\test_acquisition\test_massive_fx_flat.py`**: 12 unit tests covering CSV parsing, schema match, dtype enforcement, malformed-line resilience, sort + dedup, atomic write, frozen dataclass, month-grouping helper.

### Documentation
- **`C:\Users\qwqw1\Dropbox\cs\github\Homeguard\docs\reference\DATA_INVENTORY.md`**: updated FX section with new totals (55 symbols, 9,903 partitions, 284.6M rows, 2010-01 → 2026-05). Recorded:
...
### Credentials
- **`C:\Users\qwqw1\Dropbox\cs\github\Homeguard\.env`** (gitignored): added `MASSIVE_S3_ACCESS_KEY`, `MASSIVE_S3_SECRET_KEY`, `MASSIVE_S3_ENDPOINT=https://files.massive.com`, `MASSIVE_S3_BUCKET=flatfiles`.
...
All 7 pairs exact match: columns `[timestamp, open, high, low, close, volume, trade_count, vwap]`, dtypes `[Datetime[ns, UTC], Float64×5, Int64×2, Float64]`. Verified via `C:\Users\qwqw1\Dropbox\cs\github\Homeguard\tests\data\test_acquisition\test_massive_fx_flat.py` (12/12 passing) plus a side-by-side polars `read_parquet` comparison.
...
### Validation script
`C:\Users\qwqw1\Dropbox\cs\github\Homeguard\scripts\scratch\check_new_fx_data.py` runs all of the above checks. Re-runnable any time to re-validate.
...
2. Referenced `config/data/fx_universe.yaml` (doesn't exist; created `C:\Users\qwqw1\Dropbox\cs\github\Homeguard\config\universes\fx-2026.csv` matching existing universe-CSV convention)
...
- **Dukascopy gap-fill** for 2019-09 multi-pair outage and EURUSD 2020-10/11. Need `dukascopy-python` dep and a new plugin. Source-of-truth comparison from `C:\Users\qwqw1\Dropbox\cs\github\Homeguard\scripts\scratch\check_fx_anomalies.py` already identifies the gap windows.
- **FX validation domain** (`C:\Users\qwqw1\Dropbox\cs\github\Homeguard\src\data\validation\fx\`). Currently a placeholder docstring. Mirror `C:\Users\qwqw1\Dropbox\cs\github\Homeguard\src\data\validation\futures\` structure. Cross-rate triangulation logic from `C:\Users\qwqw1\Dropbox\cs\github\Homeguard\scripts\scratch\check_new_fx_data.py` is ready to productionize.
...
- SGDJPY has only 6 years of history vs 16 for the others. Strategies should honor `effective_start_date` from `C:\Users\qwqw1\Dropbox\cs\github\Homeguard\config\universes\fx-2026.csv` to avoid backtest bias.
