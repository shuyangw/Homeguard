...
- **Does NOT silence the `[!] [Routing] Strategy 'cscm' references unknown broker 'coinbase', using default` warning.** Initially assumed it would, but the warning fires at the routing layer (`src/trading/config/broker_routing.py:118-121`), not the factory. The routing layer builds its `broker_instances` dict only from entries under `brokers:` in `config/trading/broker_routing.yaml`, and that file does not list `coinbase` -- only `alpaca` and `ibkr`. So even with the factory now knowing about coinbase, the routing dict is unchanged and the CSCM mapping still falls back to default with a warning. Silencing it would require either (a) adding a `coinbase:` entry to `broker_routing.yaml` AND setting `COINBASE_API_KEY`/`COINBASE_API_SECRET` in `.env` so instantiation succeeds, or (b) removing the CSCM entry from `broker_routing.yaml` since `CSCMLiveAdapter` uses `CryptoBrokerRouter` directly and ignores this config map anyway. This commit does neither -- it just makes the factory itself capable of creating coinbase instances, which is the necessary-but-not-sufficient first step toward (a).
...
## Commits
- `666763e` fix(ops): close out 2026-04-23 audit (coinbase, MKR, log, session-log)
...
- **"references unknown broker" warning still fires**: see the Changes Made note on broker_factory.py. Smallest next step to silence: drop CSCM from `config/trading/broker_routing.yaml` since `CSCMLiveAdapter` doesn't honor that map anyway. Larger better step: change the warning at `broker_routing.py:118-121` to only fire for strategies actually running in the current process.
...
- `sudo journalctl -u homeguard-multi -n 200 | grep "references unknown broker"` -- **still fires once at startup** (see Changes Made note above); benign, logged as known issue rather than silenced in this commit.
