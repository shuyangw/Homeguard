"""
Proof-of-concept script for regime-based testing and walk-forward validation.
...
Run this script to test strategies with advanced validation techniques.

OPTIMIZED VERSION:
- Shorter date range (2 years instead of 4) for faster execution
- Smaller parameter grid (4 combinations instead of 9)
- Shorter walk-forward windows (6 months train instead of 12)
...
def get_market_data():
    ...
    if _cached_market_data is None:
        logger.info(f"Loading market data for {SYMBOL} ({START_DATE} to {END_DATE})...")
        start_time = time.time()
        engine = get_engine()
        _cached_market_data = engine.data_loader.load_symbols(
            [SYMBOL],
            START_DATE,
            END_DATE
        )
        elapsed = time.time() - start_time
        logger.success(f"Market data loaded in {elapsed:.1f}s")
...
def example_1_walk_forward_validation():
    ...
    Optimizes parameters on training data, tests on unseen test data.

    OPTIMIZED: Uses 6-month train, 3-month test, 2-year period
    """
    logger.blank()
    logger.separator()
    logger.header("EXAMPLE 1: WALK-FORWARD VALIDATION")
    logger.separator()
    logger.blank()
    ...
    validator = WalkForwardValidator(
        engine=engine,
        train_months=6,   # OPTIMIZED: 6 months (was 12)
    ...
    param_grid = {
        'fast_window': [10, 20],      # OPTIMIZED: 2 values (was 3)
        'slow_window': [50, 100]      # OPTIMIZED: 2 values (was 3)
    }
...
def example_2_regime_analysis():
    ...
    OPTIMIZED: Uses cached data, 2-year period
    """
    logger.blank()
    logger.separator()
    logger.header("EXAMPLE 2: REGIME-BASED ANALYSIS")
    logger.separator()
    logger.blank()
    ...
    market_prices = market_data.xs(SYMBOL, level='symbol')['close']

    # IMPORTANT: Resample to daily for regime detection (intraday data creates too many regimes)
    ...
    logger.info(f"Resampled from {len(market_prices)} intraday bars to {len(daily_prices)} daily bars")
    ...
    # Analyze by regime (use daily prices)
    regime_results = analyzer.analyze(
        portfolio_returns=returns,
        market_prices=daily_prices,
        trades=None  # Could pass trades for trade-level analysis
    )
...
def example_3_combined_analysis():
    ...
    2. Regime analysis on out-of-sample results to check robustness

    OPTIMIZED: Uses 6-month train, cached data, 2-year period
    """
    logger.blank()
    logger.separator()
    logger.header("EXAMPLE 3: COMBINED WALK-FORWARD + REGIME ANALYSIS")
    logger.separator()
    logger.blank()
    ...
    # IMPORTANT: Resample to daily for regime detection
    logger.info("Resampling market data to daily frequency...")
    daily_prices = market_prices.resample('D').last().dropna()

    # Create regime analyzer
    analyzer = RegimeAnalyzer()

    # Analyze out-of-sample returns by regime (use daily prices)
    regime_results = analyzer.analyze(
...
def main():
    ...
    logger.blank()
    logger.separator()
    logger.header("REGIME-BASED TESTING PROOF-OF-CONCEPT (OPTIMIZED)")
    logger.separator()
    ...
    logger.info("  - Shorter date range (2 years: 2022-2023)")
    logger.info("  - Smaller parameter grid (4 combinations instead of 9)")
    logger.info("  - Shorter walk-forward windows (6-month train)")
    logger.info("  - Cached data loading")
    ...
    if len(sys.argv) > 1:
        ...
        if example_num == '1':
            ...
            logger.info("Usage: python regime_analysis_example.py [1|2|3|all]")
