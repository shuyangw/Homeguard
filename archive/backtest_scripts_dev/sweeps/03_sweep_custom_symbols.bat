@echo off
REM Sweep Backtest Using Custom Symbols File
REM Tests strategy across symbols loaded from a custom file
...
REM Create example watchlist if it doesn't exist
if not exist "%~dp0..\my_watchlist.txt" (
...
python "%~dp0..\..\src\backtest_runner.py" ^
  ...
  --symbols-file "%~dp0..\my_watchlist.txt" ^
  --sweep ^
  %BACKTEST_PARALLEL% ^
