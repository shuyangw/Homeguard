...
- **Persist in the search; be honest in the verdict.** The end goal -- a profitable, deployable strategy in SOME asset class -- does not lapse, and it applies to every asset class, not just the one in front of you. So do not abandon a hypothesis on thin evidence: a single naive or in-sample screen is not a verdict. Give each mechanism a genuine, thorough effort first -- its pre-registered/enhanced variants, a real parameter sweep, walk-forward with purge/embargo, cost sensitivity, regime robustness -- before concluding it is dead. BUT "try hard to make it work" means exhausting the theory-justified variants of the MECHANISM under honest validation; it NEVER means torturing the data, lowering the bar, re-running until a gate passes by chance, or engineering around a genuine failure (each attempt is a trial and deflates the DSR -- count it, per "Count trials honestly"). Once a mechanism robustly fails net of costs under honest walk-forward and deflation, that verdict is final and IS a success (per above) -- record it and move to the next hypothesis. Thoroughness before concluding; honesty in concluding. The line between persistence and overfitting is: are you testing pre-registered economic ideas, or searching for a number?
- **A negative bounds the SPECIFICATION you tested, not the asset class.** Every liquid asset class has many profitable participants, so "this asset class has no edge / is exhausted" is a near-zero-prior claim -- if a handful of backtests seem to imply it, the test is far more likely mis-specified than the market is dead. The tell you are over-generalizing: your negative, if taken as true, would imply an entire profitable industry cannot exist. A negative result is valid and honest ONLY at the exact scope you tested -- a frequency, an execution/cost style, a signal family, a universe, a horizon -- and must NOT be inflated to the asset class. This SHARPENS "surfacing a failure is success": "this retail, daily, spread-taking, standard-factor spec dies after costs" is a real completed finding; "FX has no edge" is not a finding, it is an over-generalization. Before shelving a whole asset class, enumerate how its profitable participants ACTUALLY make money and check which of those mechanisms you tested; the untested ones are live hypotheses, not "exhaustion." Highest-leverage mis-specifications to check first, because each can flip a verdict: (1) COST SIDE -- did you only test as a spread-TAKER? A large share of real edge is EARNED as a liquidity provider/maker, where the spread flips from a cost to revenue (replaced by adverse-selection and inventory risk, often a smaller drag), so the same signal that "dies after costs" as a taker can be positive as a maker; (2) FREQUENCY -- daily/session sampling is structurally blind to microstructural edge; (3) SIGNAL FAMILY -- price/rate factors are a thin slice next to order-flow, positioning (COT, risk-reversal skew), options-implied, and cross-venue/triangular signals; (4) execution venue/latency, financing cost, and scale. This is NOT license to re-run dead specs, lower the bar, or refuse a genuine negative -- a MECHANISM that robustly fails under honest validation is still done (per "Persist in the search"). It is a rule about SCOPING the claim honestly and about spanning the edge-bearing structures of an asset class before concluding it is unproductive. Some real edges (co-located HFT, bank flow internalization, prime-broker financing) are genuinely inaccessible to this apparatus -- name those honestly rather than pretending to replicate them, but do not let their inaccessibility masquerade as "the asset class has no edge."
- **Actively hunt for p-hacking -- in the results AND in yourself.** P-hacking is mining researcher degrees of freedom until a number passes, then presenting it as one clean test. Tells to flag and refuse: reporting only the best spec out of many (selective reporting / undercounted N); picking the date range, universe, pair subset, cost model, rebalance cadence, or walk-forward window that makes it pass (specification search); dropping "outlier" periods or pairs that hurt; stopping the search the instant a gate passes (optional stopping); peeking at or refitting on the frozen held-out set; inventing the economic rationale AFTER seeing the result (HARKing); or tuning the gate threshold or the trial count N to pass. You generate specifications tirelessly, which makes you a p-hacking engine BY DEFAULT -- so before reporting any PASS, self-audit out loud: how many specs did the search actually touch, is N fully counted, was the mechanism pre-registered, did the test set stay frozen? If a result survives only via one of these moves, say so and kill it. A near-miss that needed three degrees of freedom to reach the gate is a FAIL, not a "promising lead."
...
- **Build vs verdict boundary (superpowers does NOT exempt you from strategy-lead).** The superpowers flow (brainstorming -> writing-plans -> subagent-driven-development) governs *building and refactoring* -- engines, wrappers, data plumbing. But the moment a task/phase RUNS a backtest, walk-forward, statistical gate (PSR/DSR/PBO), or smoke that produces a strategy VERDICT, that is strategy testing and MUST go through `strategy-lead`, even inside a superpowers plan. When `writing-plans` produces a plan with a verdict/gate/smoke phase, split it on that line: build tasks execute via subagent-driven-development; verdict phases are delegated to `strategy-lead`. (This is enforced, not just advised: a `PreToolUse` hook -- `.claude/hooks/strategy_lead_gate.py`, registered in `.claude/settings.json` -- hard-blocks backtest/gate/smoke commands unless `strategy-lead` has set the `.claude/.strategy-lead-active` sentinel. If a backtest is blocked, that is the guard telling you to route through `strategy-lead`.)
...
- **Fill logging is MANDATORY for every backtest, every asset class.** Every simulated run persists its fills -- never discard them. Walk-forward / sweep / multi-leg runners MUST build a run-scoped `FillSink` (`src/backtesting/engine/fill_sink.py`) -> `output/backtests/<strategy>/runs/<run_id>/` holding per-window/per-cost-leg gzip fill files, a `manifest.csv`, and (walk-forward) a `trades_oos.csv.gz` sliced to each window's OOS `[test_start, test_end)` segment via `sink.set_oos_range(window, test_start, test_end)` + `sink.finalize(oos_cfg_hash=...)`. A single-window verdict keeps the plain `output/backtests/<strategy>/<start>_to_<end>/trades.csv`. ANY new backtest/walk-forward/gate runner MUST wire this at build time -- mirror `scripts/backtest_scripts/run_fx_walkforward.py` (FX, `run_fx_backtest`) or `run_carver_walkforward.py` (futures, `run_futures_backtest`); reuse the asset-agnostic `walkforward_common` helpers (`_build_windows`, `_oos_returns_dated`, `_stitch_oos_dedup`). `strategy-lead` verifies the fills artifact exists before accepting a walk-forward verdict. Full mandate: `.claude/rules/strategy-pipeline.md` + `docs/methodology/backtesting.md` Section 12.
