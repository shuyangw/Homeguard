@echo off
REM RSI Mean Reversion Strategy (SWEEP MODE)
REM Tests RSI oversold/overbought levels across FAANG stocks
...
echo ========================================
echo RSI Mean Reversion Backtest (SWEEP)
...
python "%~dp0..\..\src\backtest_runner.py" ^
  ...
  --fees %BACKTEST_FEES% ^
  --run-name "02_rsi_mean_reversion_sweep" ^
