"""
Optimization script for MovingAverageCrossover strategy on BULL MARKET period (2019-2021).

Previous testing (2022-2024) showed negative Sharpe due to bear/choppy market.
This test validates hypothesis that MA crossover IS profitable in trending bull markets.
"""
...
def optimize_ma_crossover_bull():
    """Optimize MovingAverageCrossover strategy on bull market period."""
    logger.blank()
    logger.separator()
    logger.header("MA CROSSOVER OPTIMIZATION - BULL MARKET (2019-2021)")
    logger.separator()
    logger.blank()

    # Symbols: Large cap tech stocks that trended well in 2019-2021
    symbols = ['AAPL', 'MSFT', 'SPY']

    # Expanded parameter grid (based on previous learnings)
    param_grid = {
    ...
    total_combos = (len(param_grid['fast_window']) *
                   len(param_grid['slow_window']) *
                   len(param_grid['ma_type']))
    ...
    logger.blank()
    logger.warning("Hypothesis: Expecting POSITIVE Sharpe ratios in bull market")
    logger.blank()
    ...
    for symbol in symbols:
        ...
        try:
            # Run optimization on BULL MARKET PERIOD (2019-2021)
            ...
            if result['best_value'] > 1.0:
                logger.success(f"EXCELLENT: Sharpe > 1.0 - Strategy is profitable in bull market!")
            elif result['best_value'] > 0.5:
                logger.info(f"GOOD: Sharpe > 0.5 - Strategy shows promise")
            elif result['best_value'] > 0.0:
                logger.warning(f"MARGINAL: Sharpe > 0 but < 0.5 - Weak edge")
            else:
                logger.error(f"NEGATIVE: Sharpe < 0 - Still unprofitable in bull market")
    ...
    for symbol, result in results.items():
        if 'error' in result:
            ...
            logger.info(f"  Valid combos: {result['valid_combos']}")
        logger.blank()
    ...
    if valid_results:
        ...
        # Compare with previous bear market results
        logger.separator()
        logger.header("COMPARISON WITH PREVIOUS BEAR MARKET (2022-2024)")
        logger.separator()
        logger.blank()
        logger.info(f"Previous Bear Market Sharpe (AAPL): -2.15")
        logger.info(f"Current Bull Market Sharpe ({best_symbol}): {valid_results[best_symbol]['best_sharpe']:.4f}")
        improvement = valid_results[best_symbol]['best_sharpe'] - (-2.15)
        logger.profit(f"Improvement: {improvement:.4f} Sharpe points")
        logger.blank()
        logger.success("VALIDATION: MA Crossover performance IS regime-dependent")
        logger.blank()
...
if __name__ == '__main__':
    try:
        results = optimize_ma_crossover_bull()
